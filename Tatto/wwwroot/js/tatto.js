	...
	if (rev != parseInt(cpf.charAt(10)))
		return false;
        if (consultarDados.consultarCpf(cpf, usuario) == true) { // Grava no Cpf sem IdUsuario um novo IdUsuario.
        return true;
    }
	return true;
}
    ...
    function erroCpf(cpfUsuario) { // apresenta erro na tela baseado na função verificarCPF(cpf) 
    alert("Esse Cpf: " + cpfUsuario + " é invalido");
...
class ConsultarDados {
    ...
    consultarCpf(cpfUsuario, usuario) {
        $.ajax({ // requisição AJAX com JavaScript e jQuery
            url: '/'+usuario+'/validarcpf',
            type: 'GET',
            data: { cpfUsuario: cpfUsuario },
            success: function (result) {
                if (result == "True") {
                    alert("Esse Cpf: " + cpfUsuario + " Já está cadastrado no Sistema");
                    document.getElementById('cpf').value = (" ");
                }
    ...
    SetTipoUsuario(tipeUser) {
        $.ajax({
            type: "POST",
            url: '/Home/pegarUsuarioAtual',
            data: { tipeUser: tipeUser}
        })
    }

    consultarEmail(emailUsuario, usuario) {
        $.ajax({ // requisição AJAX com JavaScript e jQuery
            ...
            success: function (result) {
                if (result == "True") {
                    alert("Esse Email: " + emailUsuario + " Já está cadastrado no Sistema");
